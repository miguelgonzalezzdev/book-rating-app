...
export function ReviewModal({ isOpen, bookId = "", onClose }: ConfirmModalProps) {
    const { bookName, authorName, rating, setRating, reviewText, imageUrl, handleBookName, handleAuthorName, handleReviewText, handleImageSelected, handleSubmitReview } = useReview({ bookId })
    
    return (
        <div className={`fixed inset-0 backdrop-brightness-35 backdrop-blur-xs flex items-center justify-center z-50 px-2 ${!isOpen ? 'hidden' : ''}`}>
            <div className="w-full max-w-lg bg-white dark:bg-neutral-600 rounded-2xl p-4 md:p-6 shadow-lg border border-gray-200 dark:border-neutral-700">
                <div className="space-y-4 md:space-y-6">
                    <div className="w-full flex items-center md:items-stretch justify-center flex-col md:flex-row gap-2 md:gap-6">
                        {bookId && (
                            <div className="w-24 md:w-42 aspect-[3/4] overflow-hidden">
                                <img
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-4 py-3 rounded-lg min-h-20 sm:min-h-30 md:min-h-50 bg-neutral-200 dark:bg-neutral-700 text-neutral-900 dark:text-neutral-50 placeholder-gray-400 dark:placeholder-neutral-400 focus:outline-none border border-neutral-300 dark:border-transparent"
