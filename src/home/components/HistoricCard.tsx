...
export const HistoricCard = ({ historic }: { historic: Historic }) => {
    const { userName, imgProfile } = historic.user;  
    const { text, idReview, imgReview, dateTimeReview } = historic.review; 
    const { id, type, actionText, dateTime } = historic; 
        ...
        <div key={id} className="flex flex-col place-content-end p-4 md:p-6 dark:bg-neutral-600 rounded-2xl shadow-md border border-gray-200 dark:border-neutral-700 w-full lg:w-300 h-75">
            <div className="flex items-center gap-3 mb-auto">
                ...
                <p className="text-md dark:text-neutral-50">
                    <span className="dark:text-neutral-50 font-bold cursor-pointer">{userName}</span> {actionText}
                </p>
            </div>
            <div className="flex flex-row gap-6 my-4 mx-2 md:mx-10 lg:mx-20 p-3 sm:p-4 md:p-6 border border-2 border-gray-200 dark:border-neutral-500">
                <img 
