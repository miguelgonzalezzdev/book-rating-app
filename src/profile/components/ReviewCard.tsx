    ...
    return (
        <div key={review.id} className="flex flex-col gap-4 p-4 md:p-6 bg-white dark:bg-neutral-600 rounded-2xl shadow-md border border-gray-200 dark:border-neutral-700 w-full h-full">
            <div className="w-full flex justify-start gap-4">
                <div onClick={handleClick} className={`w-14 md:w-20 aspect-[3/4] overflow-hidden border-neutral-300 dark:border-neutral-500 ${review.book_id ? 'cursor-pointer' : ''}`}>
                    <img
                ...
                <div className="flex flex-col">
                    <p onClick={handleClick} className={`text-lg font-semibold text-neutral-900 dark:text-neutral-50 ${review.book_id ? 'cursor-pointer' : ''}`}>{review.title}</p>
                    <p onClick={handleClick} className={`text-md text-neutral-700 dark:text-neutral-300 italic ${review.book_id ? 'cursor-pointer' : ''}`}>{review.author}</p>
                </div>
            </div>
            ...
            <p className="text-md text-neutral-700 dark:text-neutral-300 italic">{review.book_id!=null ? "Disponible en Biblioclase" : "No disponible en Biblioclase"}</p>
        </div>
    )
}
